...
TEST_CASE("HttpHeaderParserTest", "HttpHeaderParserTest")
{
	...
	SECTION("parserTest")
	{
		...
		{
			parser.setRawHeader( testCase.header );

			REQUIRE( parser.getFieldName() == testCase.name );
			REQUIRE( parser.getFieldValue() == testCase.value );
		} // testCase : listOfTests
